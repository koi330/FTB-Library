...
public class FTBLibClientConfig {
	...
	public static boolean sync() {
		...
		item_ore_names = config.get(Configuration.CATEGORY_GENERAL, "item_ore_names", false, "Show item Ore Dictionary names in inventory.").getBoolean();
		item_nbt = config.get(Configuration.CATEGORY_GENERAL, "item_nbt", false, "Show item NBT in inventory.").getBoolean();
		debug_helper = config.get(Configuration.CATEGORY_GENERAL, "debug_helper", true, "Show help text while holding F3.").getBoolean();
		replace_vanilla_status_messages = config.get(Configuration.CATEGORY_GENERAL, "replace_vanilla_status_messages", true, "Replace vanilla status message with Notifications, which support colors and timers.").getBoolean();
