...
public class GuiManagePlayersBase extends GuiButtonListBase {
	static class ButtonPlayerBase extends SimpleTextButton {
		...
		final void updateIcon() {
			// Variant 2:
			// setIcon(gui.getTheme().getWidget(false).withColorAndBorder(getPlayerColor(),
			// 1).combineWith(gui.getTheme().getSlot(false).withColorAndBorder(Color4I.DARK_GRAY,
			// 3), new PlayerHeadIcon(entry.name).withBorder(4)));
			setIcon(new PlayerHeadIcon(entry.uuid).withBorder(getPlayerColor(), false)
