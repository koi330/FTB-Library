...
public class SimpleToast implements IToast {
	...
	public Visibility draw(GuiToast gui, long delta) {
		...
		if (list.size() == 1) {
			...
			if (delta < 1500L) {
				int k = MathHelper.floor_float(MathHelper.clamp_float((float) (1500L - delta) / 300F, 0F, 1F) * 255F) << 24
						| 67108864;
				mc.fontRenderer.drawString(getTitle(), 30, 11, i | k);
			} else {
				int i1 = MathHelper.floor_float(MathHelper.clamp_float((float) (delta - 1500L) / 300F, 0F, 1F) * 252F) << 24
						| 67108864;
