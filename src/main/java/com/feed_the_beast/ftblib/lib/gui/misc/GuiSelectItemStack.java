...
public class GuiSelectItemStack extends GuiBase {
	...
	private class ButtonSwitchMode extends Button {
		...
		@Override
		public String getTitle() {
			return I18n.format("ftblib.select_item.list_mode",
					...
					+ EnumChatFormatting.DARK_GRAY + " [" + (panelStacks.widgets.size() - 1) + "]";
		}
	...
	private class ButtonMeta extends ButtonStackConfig {
		public ButtonMeta(Panel panel) {
			super(panel, I18n.format("ftblib.select_item.meta"),
	...
	private class ButtonDisplayName extends ButtonStackConfig {
		...
		public void onClicked(MouseButton button) {
			GuiHelper.playClickSound();
			new GuiEditConfigValue("name", new ConfigString(selected.hasDisplayName() ? selected.getDisplayName() : ""),
					this).openGui();
		}
	...
	public void drawBackground(Theme theme, int x, int y, int w, int h) {
		...
		if (newStackWidgets != null) {
			...
			scrollBar.setPosAndSize(panelStacks.posX + panelStacks.width + 6, panelStacks.posY - 1, 16,
