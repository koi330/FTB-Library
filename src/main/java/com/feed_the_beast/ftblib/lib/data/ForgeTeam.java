...
public class ForgeTeam extends FinalIDObject implements INBTSerializable<NBTTagCompound>, IConfigCallback {
	...
	public boolean isRequestingInvite(@Nullable ForgePlayer player) {
		return player != null && isValid() && !isMember(player) && requestingInvite.contains(player)
				&& !isEnemy(player);
	}
	...
	public ConfigGroup getSettings() {
		if (cachedConfig == null) {
			...
			cachedConfig.setDisplayName(new ChatComponentTranslation("gui.settings").appendSibling(StringUtils.bold(
					StringUtils.color(new ChatComponentText(" #" + toString()), EnumChatFormatting.DARK_GRAY), false)));
			...
			main.setDisplayName(new ChatComponentText(FTBLib.MOD_NAME));
			...
			display.addEnum("fake_player_status", () -> fakePlayerStatus, v -> fakePlayerStatus = v,
