...
public class FTBLibTeamGuiActions {
	...
	public static final TeamAction MEMBERS = new TeamAction(FTBLib.MOD_ID, "members", GuiIcons.FRIENDS, 30) {
		...
		public void onAction(ForgePlayer player, NBTTagCompound data) {
			if (data.hasNoTags()) {
				new MessageMyTeamPlayerList(getId(), player, NO_ENEMIES_PREDICATE)
			...
			switch (data.getString("action")) {
				...
				case "invite": {
					...
					if (player.team.isRequestingInvite(p)) {
						...
						IChatComponent component = new ChatComponentTranslation("ftblib.lang.team.invited_you",
								player.team, player.getDisplayName());
	...
	public static final TeamAction ALLIES = new TeamAction(FTBLib.MOD_ID, "allies", GuiIcons.STAR, 40) {
		...
		public void onAction(ForgePlayer player, NBTTagCompound data) {
			if (data.hasNoTags()) {
				new MessageMyTeamPlayerList(getId(), player, ALLIES_PREDICATE)
						.sendTo(player.getPlayer());
			}
	...
	public static final TeamAction MODERATORS = new TeamAction(FTBLib.MOD_ID, "moderators", GuiIcons.SHIELD, 50) {
		...
		public void onAction(ForgePlayer player, NBTTagCompound data) {
			if (data.hasNoTags()) {
				new MessageMyTeamPlayerList(getId(), player, MEMBERS_PREDICATE)
						.sendTo(player.getPlayer());
				return;
			}
	...
	public static final TeamAction ENEMIES = new TeamAction(FTBLib.MOD_ID, "enemies", GuiIcons.CLOSE, 60) {
		...
		public void onAction(ForgePlayer player, NBTTagCompound data) {
			if (data.hasNoTags()) {
				new MessageMyTeamPlayerList(getId(), player, ENEMIES_PREDICATE)
						.sendTo(player.getPlayer());
			}
	...
	public static final TeamAction TRANSFER_OWNERSHIP = new TeamAction(FTBLib.MOD_ID, "transfer_ownership",
			GuiIcons.RIGHT, 10000) {
		@Override
		public Type getType(ForgePlayer player, NBTTagCompound data) {
