...
import com.feed_the_beast.ftblib.lib.util.text_components.ChatComponentScore;
...
public class JsonUtils {
	...
	public static void toJson(File file, @Nullable JsonElement element, boolean prettyPrinting) {
		try (OutputStreamWriter output = new OutputStreamWriter(new FileOutputStream(FileUtils.newFile(file)),
			...
			if (json.has("text") || json.has("notification")) {
				...
				if (json.has("notification") || json.has("timer") || json.has("important")) {
					Notification n = Notification
							.of(json.has("notification") ? new ResourceLocation(json.get("notification").getAsString())
									: Notification.VANILLA_STATUS, s);
					component = n;
				...
				component = new ChatComponentScore(net.minecraft.util.JsonUtils.getJsonObjectStringFieldValue(o1, "name"),
						net.minecraft.util.JsonUtils.getJsonObjectStringFieldValue(o1, "objective"));

				if (o1.has("value")) {
					((ChatComponentScore) component).setValue(net.minecraft.util.JsonUtils.getJsonObjectStringFieldValue(o1, "value"));
				}
			// } else if (json.has("selector")) {
			// 	component = new TextComponentSelector(net.minecraft.util.JsonUtils.getJsonObjectStringFieldValue(json, "selector"));
			// } else if (json.has("keybind")) {
			// 	component = new TextComponentKeybind(net.minecraft.util.JsonUtils.getJsonObjectStringFieldValue(json, "keybind"));
			} else {
				return null;
			}
	...
	private static void fromJsonTree0(JsonObject map, @Nullable String id0, JsonObject o) {
		for (Map.Entry<String, JsonElement> entry : o.entrySet()) {
			if (entry.getValue() instanceof JsonObject) {
				fromJsonTree0(map, (id0 == null) ? entry.getKey() : (id0 + '.' + entry.getKey()),
	...
	public static String fixJsonString(String json) {
		...
		if (json.indexOf(' ') != -1 && !((json.startsWith("\"") && json.endsWith("\""))
