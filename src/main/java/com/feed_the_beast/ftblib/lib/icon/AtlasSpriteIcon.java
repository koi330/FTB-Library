...
public class AtlasSpriteIcon extends Icon {
	...
	public void draw(int x, int y, int w, int h) {
		...
		tessellator.draw();
		// textureManager.getTexture(TextureMap.locationBlocksTexture);
	}
	...
	public IPixelBuffer createPixelBuffer() {
		try {
			...
			return PixelBuffer.from(Minecraft.getMinecraft().getResourceManager()
					.getResource(new ResourceLocation(rl.getResourceDomain(), "textures/" + rl.getResourcePath() + ".png"))
					.getInputStream());
		} catch (Exception ex) {
			return null;
		}
